...
| `go.mod` files one level deep (`*/prompts:go.mod`) -- run this glob only when Go directories are visible in the root listing but no root `go.mod` was found | Go multi-module |
